...
(function() {
	...
	*/
	function fnGetFile(pars) {
		...
		data.date = Date.parse(this.constants.FILES_SERVER[data.name]);
		...
		*/
		return fnGetCache().then(() => {
			return fnIsUpdateFile(data).then(isUpdated => {
				return ((!isUpdated || !data.cache) ? fnSendRequest(data) : data.getSavedFile(data)).then(fnGetObject, fnOnError);
			}, fnOnError);
		});
	}
	//---------------------------------
	...
	function fnIsUpdateFile(data) {
		...
		async function updateFile(_data) {
			let infoSaved = _filesClient[_data.name],
				isUpdated = infoSaved && !isNaN(_data.date) ? infoSaved.valueOf() >= _data.date : true;
			return isUpdated;
		}
	}
	...
	*/
	function fnGetSavedFile(data) {
		...
		return _currentCache.match(data.srcToRequest).then(function (response) {
