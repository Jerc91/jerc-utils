...
// Usar la información sel service worker que estar persisiteida mienntras no se actualice, para enviar la información del filesToUpdate a los workers
...
(function () {
	...
	(function () {
		...
		*/
		function fnSendRequest(request, addResponse) {
			...
			_copyRequest.filesServerToUpdate = _filesServerToUpdate;
			_copyRequest.start = performance.now();
		...
		*/
		function fnSendRequestSW(request, addResponse) {
			...
			return new Promise((resolve, reject) => {
				// Config Get
				fnExecFunction(_cbStartRequest, _copyRequest);
				sendMessageToSW(_copyRequest, data => {
					if (typeof data == 'string') {
						addResponse();
						reject(data);
						return;
					}
					else fnReseiver(data, _copyRequest, resolve, addResponse);
				});
			}).then(function(result) {
				fnExecFunction(request.then, result);   
				return result;
		...
		function sendMessageToSW(data, callback) {
			getRegistrationSW().then(serviceWorker => {
				let messageChannel = new MessageChannel();
				messageChannel.port1.onmessage = event => callback(event.data);
				serviceWorker.active.postMessage(data, [messageChannel.port2]);
			});
		}
	...
	*/
	function fnReseiver(response, request, resolve, addResponse) {
		...
		if(now - request.start > 10) {
			requestAnimationFrame(resolveRequest);
		}
		else resolveRequest();
	...
	*/
	function fnGetRequest(request, removeFunctions) {
		...
		if (typeof request === 'string' || request instanceof String) {
			return { src: request.toString(), useFS: jr.useFileSystem };
		}
		...
		request.useFS = jr.useFileSystem;
		return request;
	} // end function
	//---------------------------------
	...
	*/
	function fnReplaceWith(result, fragment, isParentElement) {
		...
		if(result.fragment) {
			if(wrapper && wrapper.firstElementChild) wrapper.firstElementChild.replaceWith(result.fragment);
